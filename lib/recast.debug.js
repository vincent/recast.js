...
function findWasmBinary() {
  return locateFile('recast.debug.wasm');
}
...
var findStringEnd = (heapOrArray, idx, maxBytesToRead, ignoreNul) => {
  ...
  function _agentPool_clear() {
        agentPoolBuffer.length = 0;
      }

  function _agentPool_get(idx, position_x, position_y, position_z, velocity_x, velocity_y, velocity_z, radius, active, state, neighbors, partial, desiredSpeed) {
  ...
  function _flush_active_agents_callback() {
  ...
  function _gl_create_object(objectName) {
...
function js_set_debug_data(dataPtr,count,mode) { const arr = count > 0 ? Array.from(HEAPF32.subarray(dataPtr >> 2, (dataPtr >> 2) + count)) : []; arr.mode = mode; Module.__RECAST_GLOBAL_DATA = arr; }
...
var wasmImports = {
  ...
  /** @export */
  agentPool_add: _agentPool_add,
  /** @export */
  agentPool_clear: _agentPool_clear,
  /** @export */
  agentPool_get: _agentPool_get,
  /** @export */
  ...
  fd_write: _fd_write,
  /** @export */
  flush_active_agents_callback: _flush_active_agents_callback,
  /** @export */
  gl_add_to_object: _gl_add_to_object,
  /** @export */
  gl_create_object: _gl_create_object,
  /** @export */
  invoke_build_callback,
  /** @export */
  invoke_file_callback,
  /** @export */
  invoke_generic_callback_string,
  /** @export */
  invoke_path_callback,
  /** @export */
  invoke_update_callback,
  /** @export */
  invoke_vector_callback,
  /** @export */
  js_set_debug_data
};

// Recast callbacks — corrected implementations replacing Emscripten's generated
// indirect-call wrappers. The naming conflict (invoke_* prefix) caused Emscripten
// to generate function-table wrappers instead of using our library_recast.js
// implementations. These replacements restore the intended behaviour.

function invoke_generic_callback_string(index, a1) {
  Module.__RECAST_CALLBACKS[index](JSON.parse(Module.UTF8ToString(a1)));
}

function invoke_vector_callback(index, a1, a2, a3) {
  Module.__RECAST_CALLBACKS[index](a1, a2, a3);
}

function invoke_path_callback(index, a1, a2) {
  var path = [];
  for (var i = 0; i < a2; i++) {
    var base = (a1 >> 2) + i * 3;
    path.push({ x: HEAPF32[base], y: HEAPF32[base + 1], z: HEAPF32[base + 2] });
  }
  Module.__RECAST_CALLBACKS[index](path);
}

function invoke_update_callback(index) {
  Module.__RECAST_CALLBACKS[index](agentPoolBuffer);
}

function invoke_build_callback(index) {
  var type = Module.UTF8ToString(index);
  Module.navmeshType = type;
  Module.vent.emit('built', type);
}

function invoke_file_callback(index, a1) {
  Module.__RECAST_CALLBACKS[index](null, FS.readFile(Module.UTF8ToString(a1)));
}
...
// end include: src/post.module.js
