    ...
    loader.load('doors.obj', function(object){
        ...
        object.traverse(function(child) {
            if (child instanceof THREE.Mesh) {
                ...
                if (child.name.match(/door/i)) {
                    geometry = child.geometry;
                    geometry.computeBoundingBox();
                    var doorSize = geometry.boundingBox.size();
                    geometry.extend = geometry.boundingBox.size().multiplyScalar(0.5);
                    doors[child.name] = child;
                    doors[child.name].closed = true;
                    doors[child.name].refs = [];
                ...
                recast.queryPolygons(
                    ...
                    recast.cb(function (polygons) {
                        ...
                        for (var i = 0; i < polygons.length; i++) {
                            door.refs.push(polygons[i].ref);
                            addMeshFromVertices(polygons[i].vertices, doorMesh, true);
                        }
                        scene.add(doorMesh);
                    })
                );
